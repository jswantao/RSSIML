...
def figure_a(signal_1d: np.ndarray, fs: float) -> tuple:
    """PSD + 幅度直方图 + 四项统计量。"""
    setup_paper_style()
    ...
    low_mask = freqs <= 30
    ax_psd.fill_between(freqs[low_mask], psd[low_mask], alpha=0.25, color="#2E7D32",
                        label="低频分量 (≤30 Hz, 人体运动)")
    ax_psd.axvline(x=20, color="#FF6F00", linestyle="--", lw=1.8,
                   label="20 Hz (Butterworth 截止)")
    ax_psd.set_xlabel("频率 (Hz)"); ax_psd.set_ylabel("功率谱密度 (V²/Hz)")
    ax_psd.set_title("(a) 功率谱密度 (PSD) — 低频分量主导", fontsize=_FONT["large"])
    ax_psd.set_xlim(0, min(fs / 2, 60)); ax_psd.legend(loc="upper right"); ax_psd.grid(alpha=0.2, which="both")

    # 下图: 幅度直方图 + 统计量
    ax_hist = fig.add_subplot(gs[1])
    ax_hist.hist(signal_1d, bins=80, color="#4472C4", alpha=0.78,
                 edgecolor="white", lw=0.3, density=True)
    ax_hist.set_xlabel("信号幅度 (去均值)"); ax_hist.set_ylabel("概率密度")
    ...
    kurt_val = float(kurtosis(signal_1d))
    ...
    fig.suptitle("CSI 信号频域与时域分布特征", fontsize=_FONT["title"], fontweight="bold")
    fig.tight_layout()
    paths = save_experiment_subfigures(fig, "CSI_FEAT", _OUT_DIR)
    return fig, paths[0], paths[1]
...
def figure_b(signal_1d: np.ndarray, fs: float) -> tuple:
    """原始波形 + 自相关 + 差分 + 能量条 + 熵曲线。"""
    setup_paper_style()
    ...
    ax_raw.set_xlabel("时间 (s)"); ax_raw.set_ylabel("幅度")
    ax_raw.set_title("(a) 原始波形 — 人体微动信号", fontsize=_FONT["large"])
    ax_raw.grid(alpha=0.15); ax_raw.set_xlim(t[0], t[-1])
    ...
    acf = np.array([np.corrcoef(signal_1d[:len(signal_1d) - lag],
                                 signal_1d[lag:])[0, 1] for lag in lags])
    ...
    ax_acf.set_xlabel("延迟 (s)"); ax_acf.set_ylabel("自相关系数")
    ax_acf.set_title("(b) 自相关函数 (ACF)", fontsize=_FONT["large"]); ax_acf.grid(alpha=0.15)
    ...
    ax_diff.axhline(y=-diff_std, color="#FF6F00", linestyle="--", lw=1.0)
    ax_diff.set_xlabel("时间 (s)"); ax_diff.set_ylabel("一阶差分")
    ax_diff.set_title("(c) 一阶差分 (逐点变化率)", fontsize=_FONT["large"])
    ax_diff.legend(fontsize=_FONT["small"]); ax_diff.grid(alpha=0.15)
    ax_diff.set_xlim(t[0], t[-1])

    # (d) 滑动窗口能量条形图
    ax_energy = fig.add_subplot(gs[2, 0])
    e_pos, e_vals = sliding_energy(signal_1d, window=int(1.0 * fs),
                                   step=int(0.5 * fs), fs=fs)
    norm_e = e_vals / (np.max(e_vals) + 1e-10)
    colors_e = plt.cm.YlOrRd(norm_e * 0.7 + 0.3)
    bar_w = (e_pos[1] - e_pos[0]) if len(e_pos) > 1 else 0.5
    ax_energy.bar(e_pos, e_vals, width=bar_w, color=colors_e, alpha=0.85,
                  edgecolor="white", lw=0.2)
    ax_energy.set_xlabel("时间 (s)"); ax_energy.set_ylabel("信号能量 (MSE)")
    ax_energy.set_title("(d) 滑动窗口能量 (窗长=1s, 步长=0.5s)", fontsize=_FONT["large"])
    ...
    ent_pos, ent_vals = sliding_entropy(signal_1d, window=int(2.0 * fs),
                                        step=int(0.25 * fs), fs=fs)
    ax_ent.plot(ent_pos, ent_vals, color="#00695C", lw=1.2, marker="o",
                ms=2, alpha=0.85)
    ax_ent.fill_between(ent_pos, ent_vals, alpha=0.18, color="#00695C")
    ax_ent.set_xlabel("时间 (s)"); ax_ent.set_ylabel("香农熵 (bits)")
    ax_ent.set_title("(e) 滑动窗口熵值 (窗长=2s, 步长=0.25s)", fontsize=_FONT["large"])
    ax_ent.grid(alpha=0.15)

    fig.suptitle("CSI 信号时域动态特征分析", fontsize=_FONT["title"], fontweight="bold")
    fig.tight_layout()
    paths = save_experiment_subfigures(fig, "CSI_FEAT", _OUT_DIR)
    return fig, {str(i): p for i, p in enumerate(paths)}
...
def main() -> None:
    ...
    filtered = butter_lowpass(signal_1d, cutoff_hz=_CUTOFF, fs=_FS, order=_ORDER)
    ...
    print(f"  {Path(pa1).name}"); print(f"  {Path(pa2).name}")
    plt.close(fig_a)
    ...
    for p in paths_b.values():
        print(f"  {Path(p).name}")
    plt.close(fig_b)
    print(f"\n全部完成, 共 {2 + len(paths_b)} 张子图保存至 {_OUT_DIR.resolve()}/")
